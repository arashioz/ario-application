...
export async function createPurchaseInvoice(data: {
  ...
  if (paidNow) {
    ...
    await SupplierDebt.create({
      supplier: data.supplier || 'شرکت',
      purchaseInvoiceId: invoice._id,
      amount: totalAmount,
      paidAmount: 0,
      date,
      notes: `بدهی فاکتور ${invoiceNumber}`,
      isSettled: false,
    });
  }
...
export async function deletePurchaseInvoice(invoiceId: string, password?: string) {
  ...
  if (debt && (debt.paidAmount || 0) > 0) {
    throw new Error('بدهی شرکت برای این فاکتور پرداخت جزئی دارد و قابل حذف نیست');
  }
  ...
  for (const item of invoice.items) {
    ...
    if (rem <= 0.01) {
      ...
      // معکوس میانگین وزنی (اگر خرید دیگری وسط نبوده)
  ...
  if (invoice.paidNow) {
    ...
    for (const tx of txs) {
      // خرید خروج بوده → برگشت یعنی افزودن به نقد
